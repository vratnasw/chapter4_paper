...
the largest ever recorded. On the other hand the Marianas subduction is among the least seismically coupled with no historic earthquake greater than 7.7 \citep{doi:10.1130/G24402A.1}\mgnote{ Can't resolve this ref wven after I do bibtex}. 
...
Our primary cross-section has the largest dimension (about 240 \mgnote{Use symbol for degrees}) and contains three subduction zones that span the range from the seismically coupled (Chile) to least coupled (Marianas). This cross section contains one subduction zone with back-arc extension. The cross-sections in Fig.\ref{fig:xsection2sumatra}B, C are smaller than in  Fig.\ref{fig:xsection2sumatra}A, and thus do not contain the coupling variability of the larger cross-section; however those cross-sections represent other subduction zones that exhibit substantial coupling (Sumatra) and very little coupling (Tonga).

\begin{table}[H]
  \caption{\mgnote{This can be retrained, but move to Discussion section and add collumns for $\Gamma$ and for inferred stresses. We will only keep entries geographically closest to our inferred coupling coifficients.}Summary of seismic coupling coefficients ($\kappa_s$) is the seismic coupling coefficient, while $\chi_g$ is the geodetic coupling coefficient \citep{scholz2012seismic}.} % title of Table
...
\centering
	...
	\begin{table}[H]
		...
		%heading
		\hline                  % inserts single horizontal line
	      	 1 &WEP& 3.042 & 148 & $100/0.82/0.77$ &  4700& \textbf{203.5} & no  \\
	        2 &WEP& 3.046 & 146 & $67.3/0.773/0.798$ & \textbf{2000} & \textbf{203.5} & no \\
	        3 &WEP& 3.08 & 141.8 & $87.3/0.703/0.7448$ & \textbf{2000} & \textbf{203.5} & yes \\
	        4 &WEP& 3.0225 & 155 & $94.6/0.8423/0.889$ & \textbf{2000} & \mgnote{VALUE?}& priors used \\
...
\includegraphics[height=35mm,width=58mm]{fig3a.pdf}%{mesh.pdf}
}
\hspace{-0.2cm}\subfigure{
...
\includegraphics[scale =0.5]{sumatra.pdf}%{mesh.pdf}
}

\hspace{0.2cm}\subfigure{
\includegraphics[scale=0.6]{middle.pdf}%{mesh.pdf}
}
%\hspace{-0.2cm}\subfigure[]{
%\includegraphics[height=35mm,width=58mm]{um_chap4.pdf}%{mesh.pdf}
%}
...
\subfigure{
\hspace{-0.8cm}\includegraphics[height=35mm,width=52mm]{fig5d.pdf}%{mesh.pdf}
}
\subfigure{
\includegraphics[height=35mm,width=52mm]{fig5e.pdf}%{mesh.pdf}
}
\hspace{0.1cm}\subfigure{
\includegraphics[height=35mm,width=52mm]{fig5f.pdf}%{mesh.pdf}
}
%\hspace{-0.2cm}\subfigure[]{
...
\mgnote{Let's Discuss. Is there a conditional distribution that does along with this (or these) inversions? Case#'s?}
...
%\includegraphics[height=35mm,width=58mm]{um_chap4.pdf}%{mesh.pdf}
%}
\caption{\mgnote{Case #'s? Are these different iterations for a single inversion} Effective viscosity for a forward model for Middle America (A) Coupled ($\Gamma = 4.0 \cdot 10^{-3}$, $\sigma_y = 160$ MPa, and $n = 3.08$) (B) Zoom in of Middle America and increase in strain rate exponent (C) ($\Gamma = 1.0 \cdot 10^{-5}$, $\sigma_y = 160$ MPa, $n = 3.08$)
...
\centering
\hspace{-0.2cm}\subfigure[]{
\includegraphics[height=35mm,width=58mm]{fig6a.pdf}%{mesh.pdf}
}
\hspace{-0.2cm}\subfigure[]{
\includegraphics[height=35mm,width=58mm]{fig6b.pdf}%{mesh.pdf}
}
\hspace{-0.2cm}\subfigure[normal stresses]{
\includegraphics[height=35mm,width=58mm]{fig6c.pdf}%{mesh.pdf}
}
\hspace{-0.2cm}\subfigure[shear stresses]{
\includegraphics[height=35mm,width=58mm]{fig6d.pdf}%{mesh.pdf}
}
\caption{
\mgnote{In A and B there is a 'dashed' line in the label box but no such line in the figures.}
(A) Normal stress comparison and 
(B) shear stress comparison for South America, Ryukyu and Izu-Bonin. 
...
\mgnote{Come by so we can discuss this section.}
