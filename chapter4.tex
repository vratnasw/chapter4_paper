...
the adjoint system \mgnote{What is $\Psi$ ?} \vrnote{done}
\begin{equation}
  \label{eq:adjoint}
  \begin{split}
...
\includegraphics[height=150mm,width=100mm]{Summary_sections_morvel56.pdf}%{mesh.pdf}
}
...
\mgnote{A discussion and presentation of the numerical solution of Rhea is needed.}\vrnote{done}
...
Unlike earlier use of a forward-adjoint Stokes optimization \citep{ratnaswamy2015adjoint} the rheological parameters were known, here they are not. 
...
In the context of earlier work \citep{ratnaswamy2015adjoint} in which conditional and marginal distributions were computed through repeated forward solves (without assumptions on their form using MCMC) and assuming they follow Gaussians, we found that the distributions were smooth and without local minima.
We test the sensitivity of inferences on the initial guess of strain rate exponent, $n$, to rule out the existance of multiple or local minimas. 
...
including the major plates as well as small plates in the back-arc (Fig.~\ref{fig:inverse1}B,C).
...
We use one of these configurations to illustrate the non-linear feedbacks which occur in these systems. Using the coupled Middle America subduction zone we start with a forward model with a typical set of parameters, $n=3.08$, $\sigma_y=160$ MPa and $\Gamma=4.0\times10^{-3}$ which gives rise to a velocity of 2.98 cm/yr compared to the observed velocity of 6 cm/yr. We see a moderate amount of shear thining below the subduction plate (Fig.\ref{fig:middle_physics}AB) and some around the slab as evident through the reduction of effectve viscosity. We see a moderate amount of shear thining below the subducting plate (Fig.\ref{fig:middle_physics}AB) and some around the slab as evident through the reduction of effectve viscosity.
However, when the coupling factor is is decreased by a factor of 10 ($\Gamma=4\cdot 10^{-5}$), the plate velocity inreases to \mgnote{value} cm/yr  and the the amount of strain rate weakening increases as evident through the much expended area of reduced effective viscosity below the plate as well as around the slab. In addition, when the coupling factor descreases, the degree of yielding within the hinge zone expands substantially Fig.\ref{fig:middle_physics}C). This interplay between the rheological parameters is important as it demonstrates a strong interaction between the global rheological parameters (yield stress and strain rate exponent) and local coupling parameter.
The forward models also point the tradeoffs that will be expected between the rheological parameters.
...
The nonlinear exponent is 3.07, the yield stress is 137 MPa and the coupling factors show a clear difference between  Ryuku and Izu (with about the same small coupling) and South America with a much larger coupling (Table~\ref{table:inversions}). The actual, dimensional stress on these shear zones requires another adjoint solve, considered below.
...
\centering
	...
	\begin{table}[H]
		\caption{Summary of inversions (bold values held fixed)} % title of Table
		\centering  % used for centering table
        \scalebox{0.8}{
		\begin{tabular}{c c c c c c c c c c c } % centered columns (2 columns)
		\hline \hline                        %inserts double horizontal lines
		Case & Subduction Zone & \textit{n} &$\sigma_y$&$\Gamma$ (SAM/RYU/IZU) $\cdot 10^{-5}$ &UM Prefactor &$E_a (kJ/mol)$&Visc. data   & Prior & Average $\eta_{UM} (10^{21})$  & $\eta_{hinge} (10^{21})$  \\ [0.5ex] % inserts table
		%heading
		\hline                  % inserts single horizontal line
...
With the stability of inversion established, we now systematically infer rheological parameters, the influence of the choice of which parameters are retained in the inversion, isolate parameter trade-offs, and determine the influence of adding additional viscosity data and prior knowledge. 
Starting with the larger cross-section (WEP), we hold fixed combinations of strain rate exponent, yield stress, activation energy and upper mantle prefactor in Cases 1-5; in all inversions the plate couplings are left free. The range of inferred strain rate exponent lies within  3.05 to 3.09, the yield stress lies within 130 to 143 MPa, and the activation energy, $E_a$, lies within 199 to 233 kJ/mol. Although in Case 6 with all of the rheological parameters left free, the non-linear exponent, yield stress and plate couplings all converge with stable values in about seven iterations (Fig.~\ref{fig:inverse1}D-F).
...
\hspace{-0.2cm}\subfigure{
\includegraphics[height=140mm,width=128mm]{fig3.pdf}%{mesh.pdf}
}
...
(B) Case 1 surface velocity at two iterations (dashed lines) compared with plate motion data (solid blue line). The velocity near the Izu plate margin shown in detail.
(C) Convergence of coupling factors  for Case 1 (solid circles) and Case 6 (open squares) for the South American (SAM in blue), Ryuku (RYU in black), and Izu (IZU in green) plate margins. 
...
\caption{Effective viscosity for two forward model for Middle America (A) Coupled ($\Gamma = 4.0 \cdot 10^{-3}$, $\sigma_y = 160$ MPa, and $n = 3.08$) (B) Zoom in of Middle America and increase in strain rate exponent (C) ($\Gamma = 1.0 \cdot 10^{-5}$, $\sigma_y = 160$ MPa, $n = 3.08$)
(B) Zoom in of Middle America subduction zone.}
...
The uncertainty of inferred rheological parameters is examined through posterior distributions as conditionals. The conditional distributions convey not only uncertainty in each parameter, trade-offs between them, but how they contribute to the underlying model physics. 
We find conditionals by using the covariance matrix of model parameters
through an inverse of the Hessian matrix (eq. \mgnote{Eq num}).



Using the conditionals for Case 1, we see a strong positive correlation between strain rate exponent and yield stress (Fig.~\ref{fig:distrib}c), a weak negative correlation between the plate couplings and yield stress (Fig.~\ref{fig:distrib}b) and a weak positive correlation between plate couplings and strain rate exponent (Fig.~\ref{fig:distrib}a). 
...
The positive correlation between the strain rate exponent and yield stress suggests that as the strain rate exponent increases, upper mantle viscosity decreases and the drag on the base of plates decrease, resulting in an increase in yield stress in order to increase the resistance in the hinge zone. 
...
With just the plate motion data, the variance on the nonlinear exponent $n$ is large wih a 95% confidence interval of \mgnote{_value_}.
...
The relationship between the plate couplings and yield stress are found in the conditional distribution (Fig. XX) where we find for each subduction zone in WEP, there is a negative correlation between the plate couplings and the yield stress. This correlation can be explained due to an increase in coupling will reduce plate motions, which will promote more dynamic weakening in the form of reducing the yield stress, so as to increase plate motions. Analyzing the relationship between the plate couplings and strain rate exponent, we similarly find that there is a positive correlation between the strain rate exponent as expected due to an increase in plate coupling (decrease in plate motions) which requires an increase in the strain rate exponent to increase plate motions.  An important observation from the conditional distributions is that there is a partitioning of the plate couplings for Izu-Bonin, Ryukyu and South America, where Izu-Bonin and Ryukyu are grouped together as decoupled, while South America is always on the coupled side.  

\subsection{Adding priors}

Prior knowledge is introduced into the inversions, with Gaussian distributions for each of the inferred parameters (Table ZZ) making sure that we do not restrict the likelihood to a particular parameter space.  Repeating a set of inversions in Cases 7-10, for the WEP cross section and determine that the form of the correlations remain invariant, including a positive correlation between the strain rate exponent and yield stress. Furthermore, we confirm the ordering of plate couplings, where South America is more coupled than Izu-Bonin and Ryukyu. With regard to the convergence of the parameters, we find that there is similar convergence to without using priors in Fig. (approximately 7 iterations).
...
Following this analysis of viscosity in the mantle, we compute the average forces on the plates in an attempt to see if such forces are also conserved. For WEP, we find that \mgnote{Why is the equation for fault normalized but the others are not. Why is it normalized by "trench depth" and not "length or depth of fault"?}
...
\subfigure[]{
\hspace{-0.8cm}\includegraphics[height=35mm,width=52mm]{fig1new.pdf}%{mesh.pdf}
}
\subfigure[]{
