...
We use the Middle America configurations to illustrate the non-linear feedbacks which occur in this plate-mantle system. Using the coupled Middle America subduction zone, we start with a forward model with a typical set of parameters, $n=3.0621$, $\sigma_y=139.1$ MPa and $\Gamma=4.0\times10^{-3}$ 
\mgnote{as discussed, we must start with an inversion Case (AB) and then perturb $\Gamma$ and discuss change inverosity and the internal dynamic state}
which gives rise to a velocity of 3.79 cm/yr compared to the observed velocity of 6.83 cm/yr. We see a moderate amount of shear thining below the subduction plate (Fig.\ref{fig:middle_physics}C) and some around the slab as evident through the reduction of effective viscosity. We see a moderate amount of shear thining below the subducting plate (Fig.\ref{fig:middle_physics}AB) and some around the slab as evident through the reduction of effective viscosity.
However, when the coupling factor is decreased by a factor of 10 ($\Gamma=4\cdot 10^{-5}$), the plate velocity inreases to 7.61 cm/yr  and the amount of strain rate weakening increases as evident through the much expanded area of reduced effective viscosity below the plate as well as around the slab. In addition, when the coupling factor decreases, the degree of yielding within the hinge zone expands substantially Fig.\ref{fig:middle_physics}D). This interplay between the rheological parameters is important and demonstrates a strong interaction between the global rheological parameters (yield stress and strain rate exponent) and local coupling parameter.
...
The Sumatra and Tonga subduction zones are of particular interest as they are thought to be among the most and least seismically coupled subduction zones, respectively.
...
\caption{ \mgnote{In the Figure, in C, IZU needs to be MAR}
...
\caption{Effective viscosity for Middle America (A) Case 32 ($\Gamma = 7.75 \cdot 10^{-5}$, $\sigma_y = 139.1$ MPa, and $n = 3.0621$) (B) Zoom in of Middle America and increase in strain rate exponent (C) ($\Gamma = 4.0 \cdot 10^{-3}$, $\sigma_y = 139.1$ MPa, $n = 3.0621$)
(D) Zoom in of Middle America subduction zone ($\Gamma = 4.0 \cdot 10^{-5}$, $\sigma_y = 139.1$ MPa, $n = 3.0621$).}
...
the MAP points of $n$ and $\sigma_y$ are very close, as discussed above, but the variance on the $n$ is reduced somewhat in the small cross sections moodels compared to the wide cross sectional WEP models.
