...
Priors are introduced with Gaussian distributions for each of the inferred parameters making sure that we do not restrict the likelihood to a particular parameter space.  Repeating a set of inversions in Cases 7-10, for the WEP cross section, we determine that the form of the correlations remain invariant, including a positive correlation between the strain rate exponent and yield stress
(Fig.~\ref{fig:activ_distrib}). Furthermore, we confirm that the ordering of plate couplings, where Chile is more coupled than Izu-Bonin and Ryukyu is also unchanged. 
The convergence of the parameters during the course of inversion
remain close to 7 iterations such that the priors do not substantially impact the overall computional work.
...
\mgnote{Add ref to Ranalli Book.  Also add: Karato and Wu, 1993, Rheology of the upper mantle: A synthesis, Science, 260, 771-778.}\vrnote{done}
...
exponents to higher valies (from $E_a=$ to ) and (from $n=3.1$ to about $3.25$, but the strain rate  exponent does not overlap with 3.5 at the 95\% confidence interval. 
...
\mgnote{We will have to address how the viscosity of slabs 
either changes or remains the same with the addition of this prior knowledge}
Only if the variance on the priors is reduced from ($5\cdot 10^{-2}$) to ($5 \cdot 10^{-3}$) are we able to recover the rock mechanics values of 3.5 and 550 kJ/mol.
\mgnote{We will need to see what the influence on the coupling factors  are when these restrictive priors are used.}
...
Fault zone stress is an extrinsic quantity and is not in our set of unknown model parameters, $\bold{m}$.
...
Using optimizations in which all of the rheological parameters are left free and with no prior knowledge or inclusion of viscosity data (Case 6 for WEP and Cases 6, 16 and 25 for Sumatra, Tonga and Middle America, respectively)
...
\mgnote{Are the normal stresses compressive or tensional?}
...
\includegraphics[scale=0.4]{activ_conditionals_summary3.pdf}%{mesh.pdf}
}
