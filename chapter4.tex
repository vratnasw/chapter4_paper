...
\begin{align*}
 ...
 \mathbb{T}(\ssigma \textbf{n})
  &= \:0 & \text{ on }\partial \Omega
  \label{eq:bc}
\end{align*}
with viscous stress tensor being  $\ssigma := 2\eta(T,n,\ssigma_y)-p\textbf{I}$ where $p$ is the forward pressure, and \mgnote{Use a different font for Tangential operator that is distinct from temperature and the Transpose}\vrnote{done}{$\mathbb{T}$ is the tangential operator ($\mathbb{T} = \textbf{I} - \textbf{n} \otimes \textbf{n}$). Furthermore, the momentum equation in ~\eqref{eq:stokes} is driven by thermal buoyancy where \textit{T} is the temperature field and \textit{Ra} is the thermal Rayleigh number. Solving the Stokes equations, we obtain the solution of the forward velocities ($\uu$) and pressures ($p$). An important part of the stress tensor $\ssigma$ is the rheological relationship. In our forward model we use a nonlinear rheology where we take into account the shear-thinning nature of the upper mantle through the use of a power law rheology with a strain rate exponent
...
\end{equation}
The normal and shear components of the stress are important as they effectively give the resisting stresses along the plate boundaries. The larger the resisting stress, the more mechanically coupled a plate boundary, and vice versa. Here, $\mathbb{T}$ and $\mathbb{N}$\mgnote{again different fonts for T}\vrnote{done} are the tangential and normal projection along the center line of the plate boundaries,
\begin{equation}
\begin{split}
        \mathbb{T} = \textbf{I} -\text n_w \otimes \text n_w\\
        \mathbb{N} = \text n_w \otimes \text n_w\\
\end{split}
\end{equation}
...
\begin{equation}
\sigma^t_{avg}=\mathbb{T}\frac{\partial\sigma}{\partial \textbf u}\cdot \textbf n
\end{equation}
with
\begin{equation}
...
\begin{equation}
\mathcal{G}:=  \mathbb{T}\frac{\partial{\ssigma}}{{\partial \mm}}\cdot \textbf{n}
\end{equation}
...
\begin{align*}
  ...
  \mathbb{T}(\hat\ssigma_\uu \textbf{n})
...
From optimizations we infer plate coupling across subduction zones and global rheological parameters such as the yield stress and strain rate exponent globally for each cross-sectional model. Compared to our earlier studies \citep{ratnaswamy2015adjoint} where we knew the actual values of the inferred parameters, 
this is not the case here. Therefore, we ran a suite of calculations to determine the influence of the initial guess has on the recovered values (strain rate exponent, Table \ref{table:initial_guess}. We vary the initial guess of the strain rate exponent from 2.0 to 3.5 as that range represents a substantial change in the amount of shear thinning in the upper mantle. From the recovered plate plate couplings and yield stress in Table \ref{table:initial_guess}, the same plate couplings and yield stress are inferred, independent of the initial guess, suggestng that we have avoided local minima for the inferred parameters. The independence on the initial guess suggests stability in the inversions such that starting far away from the \textbf{MAP} point will lead to the same set of inferences.


We now turn to the inferred plate couplings and global rheological parameters. We explore these parameter spaces to understand the covariation of rheological parameters, but if they fall within experimental bounds. We infer the plate couplings and global rheological parameters such as the yield stress and strain rate exponent in Case 1.  In all models, we choose initial values (guesses) on: (i) the  weak zone factor ($\Gamma=10^{-5}$) as such a value typically decouples the plate boundaries allowing subduction, (ii) the yield stress (125 MPa) as it provides a reasonable amount of dynamic weakening in forward models while satisfying estimates from seismicity \citep{craig2014reassessment}, (iii) strain rate exponent ($n=3.0$), a value typically associated with mantle convection and within the bounds imposed from laboratory experiments \citep{ranalli1995rheology}, and (iv) an activation energy ($E_a=203.6$kJ/mol), a value we found through multiple forward models in which plates are sufficiently strong to act as stress guides. 
...
For the WEP cross-section for Case 1, we find that dispite same initial guess for three weak zone factors, we find that the recovered values are distinctly different, with the South America subduction zone having the largest plate coupling. Furthermore, the inferred strain rate exponent suggest significant shear thinnning in the upper mantle. While the plate couplings, yield stress and strain rate exponent are important parameters to infer, a natural question to ask would be how would these parameters change if another global parameter would be included such as the upper mantle prefactor. Therefore, we infer the plate couplings, yield stress, strain rate exponent and upper mantle prefactor in Case 2. We find that the inferred couplings are different than those from Case 1 in Table \ref{table:inversions}, while the strain rate exponent is similar to what was inferred in Case 1 ($0.113\%$) as well as the yield stress in Case 2 ($1.37\%$).  

 \citep{ratnaswamy2015adjoint} Inferring the activation energy, not explored in \citep{ratnaswamy2015adjoint}, which controls the strength of temperature-depedent viscosity could help to place bounds on how strong slabs are and to what degree the weakening in the upper mantle is due to the activation energy. In Case 3, we find that the inferred activation energy is 
