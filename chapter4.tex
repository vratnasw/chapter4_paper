...
We incorporate plate motion and effective viscosity data into an optimization and derive an adjoint, gradients for the inferred parameters, and posterior distributions for  rheological paramters, stresses within plate boundaries, and the effective discosity of subducted slabs. 
We apply these methods to 2-D cross-sections of subduction zones, with temperature distributions and fault zone geometries developed from seismic and other data. We analyze the conditional and marginal distributions and find that the 
...
Variations in inferred mechanical couplings are similar to observed seismic coupling. 
...
The Chilean subduction zone is among the most seismically active with many earthquakes above 8, including the 1960 Valdivia earthquake with moment magnitude of 9.5 \citep{kanamori1974},
...
\mgnote{You need to more fully state the Forward problem, including the Stokes system, the forward variables}
The underlying physics of mantle flow is governed by the creeping of mantle rocks under geological time scales. These time scales result in the motion of the mantle behaving as a viscous flow that is governed by the Stokes equations
\begin{equation}
  \label{eq:stokes}
  \begin{split}
...
with viscous stress tensor $\ssigma := 2\eta(T,n,\ssigma_y)-p\textbf{I}$ where $p$ is the forward pressure, and \textbf{T} is the tangential operator ($\textbf{T} = \textbf{I} - \textbf{n} \otimes \textbf{n}$). Furthermore, the momentum equation in ~\eqref{eq:stokes} is driven by thermal buoyancy where \textit{T} is the input temperature field and \textit{Ra} is the thermal Rayleigh number. Solving the Stokes equations, we obtain the solutions of the forward velocities ($\uu$) and pressures ($p$). An important part of the stress tensor $\ssigma$ is the rheological relationship. In our forward model we use a nonlinear rheology where we take into account shear-thinning nature of the upper mantle through the use of a power law rheology with a strain rate exponent
and the diffusion creep in the lower mantle with a linear rheology, while using a global yield stress to allow for dynamic weakening within the hinge zones (where slabs bend). Our viscosity formulation is given in ~\eqref{eq:rheo}, where $\eta_{min}$ is the minimum effective viscosity, $\Gamma_i$ is the weak zone factor for plate margin \textit{i}, $\sigma_y$ is the yield stress, $a(T)$ is the temperature dependent component of viscosity, $n$ is the strain rate exponent and $d$ is a parameter included to regularize the solution. In ~\eqref{eq:rheo} We compute the power-law and temperature dependent part of the rheology first and take the minimum between that viscosity and the maximum viscosity ($\eta_{max}$). Dynamic weakening is taken into account by comparing the minimum between the viscosity from yielding taking the minimum viscosity. We then regularize the viscosity in ~\eqref{eq:rheo} by adding a minimum effective viscosity $\eta_{min}$.
...
We solve the nonlinear incompressible Stokes by linearizing the momentum equations, which enables us to use Newton's method \citep{rudi2015extreme}. 

 
\section{Bayesian Problem Formulation}
We cast the inverse problem in a Bayesian sense, in which we find the posterior distribution ($\ppi_{post}$) for the inferred rheological parameters such as the strain rate exponent and weakfactors. 
...
where we realize that a single set of parameters is not a unique solution, so we seek the Bayesian solution to the problem. The likelihood distribution is given as,

\begin{equation}
\ppi_{like} = \exp{\Big\{-\frac{1}{2} (\mathcal{O}\uu-\uu_{\text{obs}})^T\mathcal{C}^{-1}_{data}(\mathcal{O}\uu-\uu_{\text{obs}})}\Big\}
\label{eq:bayes}
\end{equation}
where $\mathcal{O}$ is the observation operator, $\uu_{data}$ is the observed data, $\uu$ is the results from a forward model and $\mathcal{C}_{data}$ is the covariance matrix for the observed data. The likelihood distribution gives an estimate of how well the model parameters can explain the data. However, if there is knowledge of the distribution of the inferred parameters ($\mm$), then that knowledge can be incorporated into the prior distribution ($\ppi_{prior}$).
...
and it is not apparent what distribution the prior should be.  Typically, the prior distribution is chosen to be a normal distribution as in Fig.\ref{fig:prior_ex}a 
...
The uniform distribution in Fig.~\ref{fig:prior_ex}b has the same mean; however the variance is different. Compared to a normal distribution, the variance for the uniform distribution is determined by the range of likely values, each of which has the same probability.
 ...
 For a prior described by a normal distribution, a mean, $\mu$, and covariance, $\mathcal{C}$, are needed, described as
\begin{equation}
...
Unlike the rheological parameters, we do not infer the shear and normal stress in our optimization framework. Instead a Gaussian approximation to the normal stress in Eq.~\eqref{eq:normal_shear} is constructed. A natural question would be how well the shear and normal stresses are approximated by a Gaussian distribution. Locally, near the maximum a posteriori point (\textbf{MAP}),\mgnote{This is not written in English} the is the mean of the Gaussian approximation to $\ppi_{post}$, the conditional stresses and to an extent the marginal distributions are well approximated by a Gaussian distribution \citep{ratnaswamy2015adjoint}.  We define a measure of the stress from the underlying properties such as the strain rate exponent, yield stress and so forth, e.g. $\mm$, the model parameters, as, 
\begin{equation}
\textit \ssigma = f(\mm)
\end{equation}
expanding $\ssigma$,
...
The expected value of a normal distribution $\mathcal{N}(\mu_{MAP},\sigma^2)$ is\mgnote{But, you gave this above, you need to condense: define once and refer to it},
\begin{equation}
...
For the second invariant of the stress tensor, \mgnote{write out what you mean by "one"}one will have extra terms compared to the average stress,
...
After solving for the adjoint above \mgnote{equation ?}, we then compute the gradient for ~\eqref{eq:normal_traction},
\begin{equation}
...
a(T)(\IIinv-d)^{1/(2n)}\IIinv^{-1/2}$, \mgnote{write out in words what $\Omega_w$ represents}and thus the viscosity depends
...
\end{equation}
\mgnote{Here a description of the Adjoint system and how it is derived along with definition of all of the symbols is required, with appropriate referencing to your earlier GJI paper.}With these constraints, the adjoint system becomes,
\begin{equation}
  \label{eq:adjoint}
  \begin{split}
...
\end{equation}
\mgnote{Why don't the inegrals on the right have have a differentiao, i.e. $d\Omega$ ?}
...
\end{equation}
\mgnote{Again, as above, I would find a different symbol for $\sigma$}and with a coefficient $\Gamma_i$ that was recovered in ~\eqref{eq:rheo}, $d_0$ is the center-line profile, $\sigma_{smooth}$ is the amount of smoothing for the weak zone.
...
finally the addition of the effective viscosity constraint.
...
Plate coupling, yield stress and strain rate exponent are important to mantle flow models; however the activation energy is key when it comes to controlling the amount of temperature dependence there is within these models. To this end, we allowed the activation energy in our inferences in Case 6, and find that the inferred value is $207$ kJ/mol, while obtaining a larger coupling for Sumatra compared to what was found for Izu-Bonin and Ryukyu in  Cases 1-4. Furthermore, our inferred strain rate exponent and yield stress are similar to what we obtained in Case 5, suggesting that these parameters are preferred to produce the observed plate motions. 
