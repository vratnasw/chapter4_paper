...
\mgnote{More details needed, such as the Type of elements, more description on the solver used and more references, inclusing for AMR}
...
\begin{table}[H]
		...
		%heading
		\hline                  % inserts single horizontal line
        	 2.0 &3.079 & 137 & $62.39/0.0.711/0.742$   \\
	         2.95 &3.079 & 137 & $62.4/0.7109/0.742$    \\
	        3.5 &3.079 & 137 & $62.41/0.0.711/0.742$  \\             
                \hline %inserts single line
		\end{tabular}
...
We use the Middle America configurations to illustrate the non-linear feedbacks which occur in this plate-mantle system. Using the coupled Middle America subduction zone, we start with a forward model with a typical set of parameters, $n=3.08$, $\sigma_y=160$ MPa and $\Gamma=4.0\times10^{-3}$ which gives rise to a velocity of 2.98 cm/yr compared to the observed velocity of 6.72 cm/yr. We see a moderate amount of shear thining below the subduction plate (Fig.\ref{fig:middle_physics}AB) and some around the slab as evident through the reduction of effective viscosity. We see a moderate amount of shear thining below the subducting plate (Fig.\ref{fig:middle_physics}AB) and some around the slab as evident through the reduction of effective viscosity.
However, when the coupling factor is decreased by a factor of 10 ($\Gamma=4\cdot 10^{-5}$), the plate velocity inreases to 7.44 cm/yr  and the amount of strain rate weakening increases as evident through the much expanded area of reduced effective viscosity below the plate as well as around the slab. In addition, when the coupling factor decreases, the degree of yielding within the hinge zone expands substantially Fig.\ref{fig:middle_physics}C). This interplay between the rheological parameters is important as it demonstrates a strong interaction between the global rheological parameters (yield stress and strain rate exponent) and local coupling parameter.
The forward models well illustrate the tradeoffs that will be expected between the rheological parameters.
...
The nonlinear exponent is 3.07, the yield stress is 137 MPa and the coupling factors show a clear difference between  Ryuku and Mariana (with about the same small coupling value) and South America with a much larger coupling value (Table~\ref{table:inversions}). The actual, dimensional stress on these shear zones requires another adjoint solve, considered below.
         ...
         25 &Tonga  & 3.051 & 135.2 & 0.7& 3196.1 & 225.8 &no &no &3.07& $8.53$ \\             
          ...
          30&Tonga  & 3.097 &  \textbf{120} & 0.73 &3639.7 &228.3  &no &yes &3.29& $8.40$  \\             
           31 &Tonga  & 3.083  & 142 & $0.79 $ & 3492.3  &225.8  &no &yes &1.93  & $8.39$ \\             
            32 &Middle America  & 3.0621 & 139.1 & 1.1& \textbf{2000} & \textbf{203.5} &no  &no &3.1 & $8.95$ \\               
...
Starting with the larger cross-section (WEP), we hold fixed different combinations of strain rate exponent, yield stress, activation energy and upper mantle prefactor in Cases 2-5, all variants of Case 1; in all inversions the plate couplings are left free. The range of inferred strain rate exponent lies within  3.05 to 3.09, the yield stress lies within 130 to 143 MPa, and the activation energy, $E_a$, lies within 199 to 233 kJ/mol. In the least restricted inversion (Case 6) with all of the rheological parameters left free, the non-linear exponent, yield stress and plate couplings all converge with stable values in about seven iterations (Fig.~\ref{fig:inverse1}D-F).
The inferred ordering of the coupling factors and their inferred values only varies by a small count compared to the $\sim10^2\times$ change between the low coupling of Ryuku and Mariana compared to the large value for South America.
The rheological parameters and coupling values appear to be robustly inferred with this set of input data.


The Sumatra and Tonga subduction zones are of particular interest as they are thought to be among the moost and least, respectively, seismically coupled subduction zones.
Therefore, we repeat similar sets of cases for these additional cross sections, for Sumatra, Middle America, and Tonga (Fig.~\ref{fig:xsection2sumatra})
The global rheological parameters strain-rate exponent and yield stress genenerally recover values within the range found from the WEP cross section. The only small difference is that $n$ can be as high as 0.12 for Sumatra and Middle America and the smallest yield stresses occur for Tonga.
...
(B) Case 1 surface velocity at two different iterations (dashed lines) compared with plate motion data (solid blue line). The velocity near the Mariana plate margin shown in detail.
...
\includegraphics[scale =0.4]{forwardfigure.pdf}%{mesh.pdf}
}

\caption{Effective viscosity for two forward model for Middle America (A) Coupled ($\Gamma = 4.0 \cdot 10^{-3}$, $\sigma_y = 160$ MPa, and $n = 3.08$) (B) Zoom in of Middle America for Case 15 
(C) Zoom in of Middle America subduction zone with perturbed $\Gamma = 4 \cdot 10^{-3}$.
(D) Zoom in of Middle America subduction zone with perturbed $\Gamma = 4 \cdot 10^{-5}$.}
...
The uncertainty of inferred rheological parameters is examined through posterior distributions as conditionals. The conditional distributions convey not only uncertainty in each parameter and trade-offs between them, but they show how these parameters contribute to the underlying model physics. 
...
With just the plate motion data, the variance on the nonlinear exponent $n$ is quite large with a 95\% confidence interval of $3.079 \pm 0.67$ \mgnote{write out the varience for a specific Case, like Case 1}.

The differences in the coupling factors for different subduction zones are
best shown using the conditional distributions for Case 1  (Fig.\ref{fig:distrib}B,C) and find that there is a clear demarcation between the least coupled subduction zones (Ryukyu and Marianas) and South America 
...
the MAP points of $n$ and $\sigma_y$ are very close, as discussed above, but the variance on the $n$ is reduced considerably in the small cross sections moodels compared to the wide cross sectional WEP models.
...
Middle America has a larger coupling factor compared with Tonga at greater than a 95\% confidence.

The coupling factors are intrinsic quantities, not depending on the forces in a particular subduction zone or case.
Using just the kinematic data, in conjunction with the geometry and amplitude of the driving forces,
...
Prior knowledge is introduced into the inversions, with Gaussian distributions for each of the inferred parameters (Table~\ref{table:inversions}) making sure that we do not restrict the likelihood to a particular parameter space.  Repeating a set of inversions in Cases 7-10, for the WEP cross section and determine that the form of the correlations remain invariant, including a positive correlation between the strain rate exponent and yield stress. Furthermore, we confirm the ordering of plate couplings, where South America is more coupled than Izu-Bonin and Ryukyu. With regard to the convergence of the parameters, we find that there is similar convergence to without using priors in Fig. (approximately 7 iterations).
...
\includegraphics[scale=0.4]{figure7new.pdf}%{mesh.pdf}
...
\caption{Activation energy vs. strain rate exponent for (A)WEP. 
(B) Sumatra. 
